...
public class TrangDangNhap extends JFrame implements ActionListener, MouseListener {
	...
	public TrangDangNhap() {
		try {
			...
			JLabel label_BossLogin = new JLabel("BOSS LOGIN");
			label_BossLogin.setForeground(Color.GREEN);
			label_BossLogin.setBackground(Color.YELLOW);
			label_BossLogin.setHorizontalAlignment(SwingConstants.CENTER);
			label_BossLogin.setFont(new Font("Tahoma", Font.BOLD, 30));
			label_BossLogin.setBounds(484, 331, 291, 79);
			label_BossLogin.setOpaque(true);
			getContentPane().add(label_BossLogin);
		...
		if (e.getActionCommand().equals("Đăng ký")) {
			...
			if (tenNguoiDungNhap.equals("") || matKhauCuoiCung.equals("")) {
				...
				if (check()) {
					try {
						UIManager.setLookAndFeel(UIManager.getSystemLookAndFeelClassName());
						new Captcha_tick(tenNguoiDungNhap);
					} catch (Exception e2) {
						e2.printStackTrace();
					}
					this.dispose();
